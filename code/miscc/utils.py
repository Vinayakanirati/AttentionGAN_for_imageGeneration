...
def build_super_images2(real_imgs, captions, cap_lens, ixtoword,
    ...
    for i in range(num):
        ...
        for j in range(num_attn):
            ...
            maxV = one_map.max()
            epsilon=1e-8
            one_map = (one_map - minV) / (maxV - minV + epsilon)
            row_beforeNorm.append(one_map)
        sorted_indices = np.argsort(conf_score)[::-1]

        for j in range(num_attn):
            one_map = row_beforeNorm[j]    # float array in [0,1], shape might be (H,W) or (H,W,C)
            # … your pyramid_expand, min/max normalization, etc. …
            PIL_im = Image.fromarray(np.uint8(img))
            norm_map = (one_map * 255).astype(np.uint8)
            # If it’s 3‑D (H×W×C), take first channel; if higher‑dim, squeeze out singleton axes
            if norm_map.ndim == 3:
                norm_map = norm_map[:, :, 0]
            elif norm_map.ndim > 3:
                norm_map = np.squeeze(norm_map)
            # Now create a grayscale PIL image
            PIL_att = Image.fromarray(norm_map, mode='L')

            merged = Image.new('RGBA', (vis_size, vis_size), (0, 0, 0, 0))
            mask = Image.new('L', (vis_size, vis_size), (180))
            merged.paste(PIL_im, (0, 0))
            merged.paste(PIL_att, (0, 0), mask)
            merged = np.array(merged)[:, :, :3]

            row.append(np.concatenate([merged, middle_pad], 1))
            #
